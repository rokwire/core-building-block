...
func (app *application) admCreateAppOrgGroup(name string, permissionIDs []string, rolesIDs []string, appID string, orgID string, l *logs.Log) (*model.AppOrgGroup, error) {
	...
	id, _ := uuid.NewUUID()
	now := time.Now()
	group := model.AppOrgGroup{ID: id.String(), Name: name, Roles: groupRoles, Permissions: groupPermissions, AppOrg: model.ApplicationOrganization{ID: appOrg.ID}, DateCreated: now}
