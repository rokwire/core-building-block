...
func (app *application) admGetAccounts(limit int, offset int, appID string, orgID string, accountID *string, authTypeIdentifier *string, hasPermissions *bool,
	permissions []string, roleIDs []string, groupIDs []string) ([]model.Account, error) {
	//find the accounts
	accounts, err := app.storage.FindAccounts(limit, offset, appID, orgID, accountID, authTypeIdentifier, hasPermissions, permissions, roleIDs, groupIDs)
	if err != nil {
