...
func NewAuth(serviceID string, host string, authPrivKey *rsa.PrivateKey, storage Storage, minTokenExp *int64, maxTokenExp *int64, phoneVerifyServiceID string, logger *logs.Logger) (*Auth, error) {
	if minTokenExp == nil {
	...
	auth := &Auth{storage: storage, authTypes: authTypes, authPrivKey: authPrivKey, AuthService: nil,
		...
		authConfigs: authConfigs, authConfigsLock: authConfigsLock, phoneVerifyServiceID: phoneVerifyServiceID}
