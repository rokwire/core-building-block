...
// FindAppAsset provides a mock function with given fields: orgID, appID, name
func (_m *Storage) FindAppAsset(orgID string, appID string, name string) (*model.AppAsset, error) {
	ret := _m.Called(orgID, appID, name)

	if len(ret) == 0 {
		panic("no return value specified for FindAppAsset")
	}

	var r0 *model.AppAsset
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string, string) (*model.AppAsset, error)); ok {
		return rf(orgID, appID, name)
	}
	if rf, ok := ret.Get(0).(func(string, string, string) *model.AppAsset); ok {
		r0 = rf(orgID, appID, name)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.AppAsset)
		}
	}

	if rf, ok := ret.Get(1).(func(string, string, string) error); ok {
		r1 = rf(orgID, appID, name)
	} else {
		r1 = ret.Error(1)
	}
