...
func (_m *Storage) InsertAccountPermissions(accountID string, appOrgID string, permissions []model.Permission) error {
	ret := _m.Called(accountID, appOrgID, permissions)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, string, []model.Permission) error); ok {
		r0 = rf(accountID, appOrgID, permissions)
	} else {
		r0 = ret.Error(0)
	}
