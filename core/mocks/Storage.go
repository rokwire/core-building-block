...
// DeleteVersion provides a mock function with given fields: context, versionObj, appTypeID, versionID
func (_m *Storage) DeleteVersion(context storage.TransactionContext, versionObj *model.Version, appTypeID string, versionID string) error {
	ret := _m.Called(context, versionObj, appTypeID, versionID)

	var r0 error
	if rf, ok := ret.Get(0).(func(storage.TransactionContext, *model.Version, string, string) error); ok {
		r0 = rf(context, versionObj, appTypeID, versionID)
	} else {
		r0 = ret.Error(0)
	}
