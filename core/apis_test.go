...
func BuildTestCoreAPIs(storage core.Storage) *core.APIs {
	return core.NewCoreAPIs("local", "1.1.1", "build", storage, nil, nil, nil, nil)
}
...
func TestSysGetOrganization(t *testing.T) {
	...
	// second case error
	storage2 := genmocks.Storage{}
	storage2.On("FindOrganization", "_id").Return(&model.Organization{ID: "_id"}, nil)
	coreAPIs = BuildTestCoreAPIs(&storage2)
...
func TestSysGetApplication(t *testing.T) {
	...
	// second case error
	storage2 := genmocks.Storage{}
	storage2.On("FindApplication", "_id").Return(&model.Application{ID: "_id"}, nil)
	coreAPIs = BuildTestCoreAPIs(&storage2)
...
func TestSysGetApplications(t *testing.T) {
	...
	getApplications, _ := coreAPIs.System.SysGetApplications()
	...
	err, _ := coreAPIs.System.SysGetApplications()
