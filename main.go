...
func main() {
	...
	//bb adapter
	bbApiKey := envLoader.GetAndLogEnvVar("ROKWIRE_CORE_BBS_API_KEY", false, false)
	...
	//web adapter
	webAdapter := web.NewWebAdapter(env, serviceID, auth.AuthService, port, coreAPIs, host, bbApiKey, logger)
	webAdapter.Start()
}
