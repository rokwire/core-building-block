...
func (sa *Adapter) UpdateApplicationOrganization(context TransactionContext, appOrg model.ApplicationOrganization) error {
	filter := bson.D{primitive.E{Key: "app_id", Value: appOrg.Application.ID}, primitive.E{Key: "org_id", Value: appOrg.Organization.ID}}
	update := bson.D{primitive.E{Key: "$set", Value: bson.D{
		...
		primitive.E{Key: "date_updated", Value: appOrg.DateUpdated},
	}}}

	res, err := sa.db.applicationsOrganizations.UpdateOneWithContext(context, filter, update, nil)
	if err != nil {
		return errors.WrapErrorAction(logutils.ActionInsert, model.TypeApplicationOrganization, nil, err)
	}
	if res.ModifiedCount != 1 {
