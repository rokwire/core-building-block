...
func (sa *Adapter) InsertAnonymousProfile(profile *model.AnonymousProfile) (*model.AnonymousProfile, error) {
	...
	if err != nil {
		return nil, errors.WrapErrorAction(logutils.ActionInsert, model.TypeAnonymousProfile, nil, err)
	}
	return profile, nil
}
...
func (sa *Adapter) FindAnonymousProfile(id string) (*model.AnonymousProfile, error) {
	...
	if err != nil {
		return nil, errors.WrapErrorAction(logutils.ActionFind, model.TypeAnonymousProfile, &logutils.FieldArgs{"id": id}, err)
	}
...
func (sa *Adapter) DeleteAnonymousProfile(id string) error {
	...
	if err != nil {
		return errors.WrapErrorAction(logutils.ActionDelete, model.TypeAnonymousProfile, &logutils.FieldArgs{"id": id}, err)
	}
	if result == nil {
		return errors.WrapErrorData(logutils.StatusInvalid, "result", &logutils.FieldArgs{"id": id}, err)
	}
	deletedCount := result.DeletedCount
	if deletedCount == 0 {
		return errors.WrapErrorData(logutils.StatusMissing, model.TypeServiceReg, &logutils.FieldArgs{"id": id}, err)
	}
