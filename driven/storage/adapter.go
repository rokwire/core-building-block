...
func (sa *Adapter) PerformTransaction(transaction func(context TransactionContext) error) error {
	...
	err := sa.db.dbClient.UseSession(context.Background(), func(sessionContext context.Context) error {
		...
		err = sess.CommitTransaction(sessionContext)
		if err != nil {
			sa.abortTransaction(sessionContext)
...
func (sa *Adapter) FindLoginSessionsByParams(appID string, orgID string, sessionID *string, identifier *string, accountAuthTypeIdentifier *string,
	...
	findOptions.SetLimit(limitLoginSession)
	err := sa.db.loginsSessions.Find(filter, &result, []options.Lister[options.FindOptions]{findOptions})
	if err != nil {
...
func (sa *Adapter) FindAccounts(context TransactionContext, limit *int, offset *int, appID string, orgID string, accountID *string, firstName *string, lastName *string, authType *string,
	...
	err = sa.db.tenantsAccounts.FindWithContext(context, filter, &list, []options.Lister[options.FindOptions]{findOptions})
	if err != nil {
		return nil, errors.WrapErrorAction(logutils.ActionFind, model.TypeAccount, nil, err)
	}
...
func (sa *Adapter) FindAccountsByParams(searchParams map[string]interface{}, appID string, orgID string, limit int, offset int, allAccess bool, approvedKeys []string) ([]map[string]interface{}, error) {
	...
	if !allAccess {
		findOptions.SetProjection(sa.getProjectionForKeys(approvedKeys))
	}

	err = sa.db.tenantsAccounts.Find(filter, &accounts, []options.Lister[options.FindOptions]{findOptions})
	if err != nil {
		return nil, errors.WrapErrorAction(logutils.ActionFind, model.TypeAccount, nil, err)
	}
...
func (sa *Adapter) findAccount(context TransactionContext, key string, id string, currentAppOrgID *string) (*model.Account, error) {
	...
	appsOrgs, err := sa.getCachedApplicationOrganizationByKeys(membershipsAppsOrgsIDs)
	if err != nil {
		return nil, errors.WrapErrorAction(logutils.ActionLoadCache, model.TypeApplicationOrganization, &logutils.FieldArgs{"app_org_ids": membershipsAppsOrgsIDs}, err)
	}
	if len(appsOrgs) != len(account.OrgAppsMemberships) {
		return nil, errors.WrapErrorAction(logutils.ActionCount, "does not match memberships apps orgs ids count", nil, err)
	}
...
func (sa *Adapter) updateAppOrgRole(context TransactionContext, item model.AppOrgRole) error {
	...
	accountsArrayFilters := []interface{}{
...
func (sa *Adapter) loadAppConfigs() ([]model.ApplicationConfig, error) {
	filter := bson.D{}
	findOptions := options.Find()
	findOptions.SetSort(bson.D{bson.E{Key: "app_type_id", Value: 1}, bson.E{Key: "app_org_id", Value: 1}, bson.E{Key: "version.version_numbers.major", Value: -1}, bson.E{Key: "version.version_numbers.minor", Value: -1}, bson.E{Key: "version.version_numbers.patch", Value: -1}}) //sort by version numbers
	var list []applicationConfig

	err := sa.db.applicationConfigs.Find(filter, &list, []options.Lister[options.FindOptions]{findOptions})
	if err != nil {
							...
							if listCheck && len(list) > 0 {
								for index := range list {
									...
									case string, int, int64, float64:
										complexMatch = append(complexMatch, bson.M{k: bson.M{"$elemMatch": bson.M{"$eq": v}}})
									}
								}
...
func (sa *Adapter) abortTransaction(sessionContext context.Context) {
	sess := mongo.SessionFromContext(sessionContext)
	err := sess.AbortTransaction(sessionContext)
	if err != nil {
...
// TransactionContext wraps context.Context (carrying a mongo.Session) for use by external packages
