...
func profileToStorage(item model.Profile) profile {
	return profile{ID: item.ID, PhotoURL: item.PhotoURL, PronunciationURL: item.PronunciationURL, Pronouns: item.Pronouns,
		...
		College: item.College, Department: item.Department, Major: item.Major, SecondMajor: item.SecondMajor, SecondDepartment: item.SecondDepartment,
		WorkTitle: item.WorkTitle, WorkPhone: item.WorkPhone, WorkEmail: item.WorkEmail, WorkAddress: item.WorkAddress, WorkAddress2: item.WorkAddress2,
		WorkPOBox: item.WorkPOBox, WorkCity: item.WorkCity, WorkZipCode: item.WorkZipCode, WorkState: item.WorkState, WorkCountry: item.WorkCountry}
}
