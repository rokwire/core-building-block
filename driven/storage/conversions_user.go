...
func accountFromStorage(item account, appOrg model.ApplicationOrganization) model.Account {
	...
	return model.Account{ID: id, AppOrg: appOrg, Admin: item.Admin, Permissions: permissions,
...
func accountToStorage(item *model.Account) *account {
	...
	return &account{ID: id, AppOrgID: appOrgID, Admin: item.Admin, Permissions: permissions, Roles: roles, Groups: groups,
		AuthTypes: authTypes, MFATypes: mfaTypes, ExternalIDs: item.ExternalIDs, Preferences: item.Preferences, Profile: profile,
