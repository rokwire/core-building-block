...
GITHUB_APP_CONFIG_WEBHOOK_SECRET | < string > | no | Secret to to validate GitHub webhook signatures for app configs.
GITHUB_TOKEN | < string > | no | Access token that authenticates with GitHub.
GITHUB_APP_CONFIG_ORG_NAME | < string > | no | Organization name of the app config GitHub repository.
GITHUB_APP_CONFIG_REPO_NAME | < string > | no | App config GitHub repository name.
GITHUB_APP_CONFIG_WEBHOOK_CONFIG_PATH | < string > | no | File path of webhook-config.json in the app config GitHub repository.
GITHUB_APP_CONFIG_BRANCH | < string > | no | Branch name to process webhooks for app config GitHub repository.
### Run Application
