...
func (we Adapter) Start() {
	...
	servicesSubRouter.HandleFunc("/auth/account-exists", we.wrapFunc(we.servicesApisHandler.authAccountExists, nil)).Methods("POST") //Requires API key in request
	servicesSubRouter.HandleFunc("/auth/verify", we.wrapFunc(we.servicesApisHandler.authVerifyCode, nil)).Methods("GET")             //Public (validates code)
	servicesSubRouter.HandleFunc("/auth/link-creds", we.wrapFunc(we.servicesApisHandler.authLinkCreds, we.auth.servicesAuthenticatedAuth)).Methods("POST")
	...
	servicesSubRouter.HandleFunc("/auth/service-regs", we.wrapFunc(we.servicesApisHandler.authGetServiceRegistrations, we.auth.servicesAuth)).Methods("GET")
